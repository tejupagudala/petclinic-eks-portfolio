...
To achieve that goal, we use Spring Cloud Gateway, Spring Cloud Circuit Breaker, Spring Cloud Config, Resilience4j
...
Startup of supporting services is optional.
...
Containers startup order is coordinated with the Docker Compose [depends-on](https://github.com/compose-spec/compose-spec/blob/main/spec.md#depends_on) expression.
...
Each Java application is started with the `chaos-monkey` profile by `./scripts/run_all.sh`.
